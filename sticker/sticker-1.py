...
class StickerGenerator:
    ...
    def process_image_to_data(self, input_image_path, outline_width=15):
        ...
        with open(input_image_path, 'rb') as i:
            ...
            img_rgba_pil = Image.open(io.BytesIO(output_data))
        
        # 转为 OpenCV 格式 (numpy array)
    ...
    def generate_pdf(self, process_data, output_pdf_path):
        ...
        # 描边通常用洋红色 (Magenta)
        self._draw_contours_as_path(c, outline_mask, fill=False, stroke=True, color=magenta, stroke_width=1, h_page=h_px)
        # c.endOCG()
    ...
    def _draw_contours_as_path(self, c, mask, fill=False, stroke=True, color=None, stroke_width=1, h_page=0, smooth_factor=0.002):
        """
    ...
    def generate_svg_cutline(self, mask, output_svg_path, smooth_factor=0.002):
        """
...
if __name__ == "__main__":
    ...
    input_path = "/Users/wang/code-project/marketing_material_files_and_information_processing_tools/sticker/SCR-20241128-mbke.jpeg"
    
    # 获取处理后的中间数据
    process_data = generator.process_image_to_data(input_path, outline_width=20)
    
    # 生成 PDF
    output_pdf_path = os.path.join(output_dir, "sticker_layers.pdf")
